...
export const ActivityDialog = ({ visible, showDialog, setSnackBar, activity }: Props) => {
  const { user } = useAuth();
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      if(activity){
        ...
        setSnackBar({visible: true, message: 'New activity added successfuly!', error: false})
      }
    } catch (error) {
        ...
        <Dialog visible={visible} onDismiss={dismissDialog}>
          <Dialog.Content testID='new-activity-dialog'>
            ...
            >
              ...
              />
              {errors['name'] ? <Text testID='input-qty-error' style={styles.error}>{errors['name']}</Text> : null}
              <IconPicker
                showIconPicker={showIconPicker}
                toggleIconPicker={() => setShowIconPicker(!showIconPicker)}
                iconDetails={[{ family: "MaterialCommunityIcons", icons: [
                    "shopping",
                    "gas-station",
                    "medical-bag",
                    "account",
                ...
                onSelect={(icon: any) => { setIconName(icon.icon); setShowIconPicker(false); setErrors({...errors, ['icon']: ''})}}
                content={iconName ?
                  <List.Icon icon={iconName} color={iconColor ?? 'red'}/> :
                  <View style={styles.icon}>
                    <Text style={{color: colors.primary, fontWeight: 'bold', fontSize: 15}}>Select an icon</Text>
                  </View>
                }
              />
              ...
              <View style={styles.input}>
                <Text style={{paddingBottom: 5,color: colors.text, fontWeight: 'bold', fontSize: 15}}>Include Number</Text>
                <Switch value={isQuantity} onValueChange={() => setIsQuantity(!isQuantity)} />
              </View>
              <View style={styles.input}>
                <Text style={{paddingBottom: 5,color: colors.text, fontWeight: 'bold', fontSize: 15}}>Include Note</Text>
                <Switch value={isNote} onValueChange={() => setIsNote(!isNote)} />
              </View>
              <TextInput
                testID='input-day'
                label="Day of month"
              ...
              <TextInput
                ...
                autoComplete={false}
              />
              {errors['monthDay'] ? <Text testID='input-qty-error' style={styles.error}>{errors['monthDay']}</Text> : null}
            </View>
            {Object.entries(errors).filter(([key, value]) => key !== 'name' && key !== 'monthDay').map(([key, value])=> <Text key={key} testID='input-global-error' style={styles.error}>{value}</Text>)}
          </Dialog.Content>
          <Dialog.Actions>
            ...
            {activity &&
              <Button testID='dialog-delete-activity-button' onPress={onDelete}>Delete</Button>
            }
          </Dialog.Actions>
        </Dialog>
        </Portal>
